...
-   Table 1: Summary statistics of research areas (count, frequency)
...
-   Table 2: Top 5 institutions with the most researchers in the top research field
...
Graphs are a really important visual resprestation that helps us visaulize how the datset is distributed it allows us to analyse the variety of variables found in the data and how it relates to other variables across. 
...
-  The x-axis (Institutions) and y-axis (Counts of Researchers) are labeled appropriately.
-   Pie chart: Proportion of of different research fields

```{r, echo=FALSE, eval=TRUE}
# Filter and summarize research counts for selected fields
research_distribution = research_summary %>%
  summarise(Network_and_Security = sum(`Networks and security`, na.rm = TRUE),
            Autonomy_and_AI = sum(`Autonomy and AI`, na.rm = TRUE),
            Software = sum(Software, na.rm = TRUE),
            Connected_Vehicles = sum(`Connected vehicles`, na.rm = TRUE),
            Nanotechnology = sum(Nanotechnology, na.rm = TRUE)) %>%
  pivot_longer(cols = everything(), names_to = "Research_Field", values_to =
                 "Count")

# Create a pie chart
ggplot(research_distribution, aes(x = "", y = Count, fill = Research_Field)) +
  geom_bar(stat = "identity", width = 1) +
  coord_polar("y", start = 0) +
  theme_void() +
  labs(title = "Research Distribution in top Research Fields") +
  scale_fill_brewer(palette = "Set2") 
```

-  Each slice represents the proportion of researchers working in that field, based on data aggregated from multiple institutions.
...
```{r, echo=FALSE, eval=TRUE}
 ggplot(researchers, aes(x = Tag.1)) +
  ...
  labs(title = "Frequency of Research Tag 1", x = "Research Field", y = "count")
...
t_test_result = t.test(institution_counts, conf.level = 0.9)
...
# Print results
print(boot_CI)
# Comparing with t-test confidence interval
